...
import { Send, Loader2, Mic, Square, Play, Pause } from 'lucide-react';
...
export default function ChatPanel({ userId, friendId, friendName, friendVibe }: ChatPanelProps) {
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel(`chat_panel_${[userId, friendId].sort().join('_')}`)
            .on(
                'postgres_changes',
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="border-b border-zinc-800 px-4 py-3 flex items-center gap-3 shrink-0">
                ...
                <div className="min-w-0">
                    <h2 className="font-bold text-white truncate">{friendName}</h2>
                    {friendVibe && <p className="text-xs text-zinc-500 italic truncate">"{friendVibe}"</p>}
                </div>
            </div>
